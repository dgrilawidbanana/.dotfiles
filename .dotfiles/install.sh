...
pip install --user git+https://github.com/eeriemyxi/mechvibes-lite

if id inputrunner &> /dev/null; then
	printf "\e[1;32m[*]\e[0m Adding user 'inputrunner' to group 'input'\n"
	sudo usermod -aG input inputrunner
else
	printf "\e[1;32m[*]\e[0m Adding user 'inputrunner'\n"
	sudo useradd -m -d /home/inputrunner -G input inputrunner
fi
...
sudo -iu inputrunner ln -sf "$script_dir/inputrunner.bashrc" /home/inputrunner/.bashrc
...
ALL ALL=(inputrunner) NOPASSWD: /home/inputrunner/.local/bin/mvibes wskey daemon
EOF
